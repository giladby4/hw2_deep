...
class LeakyReLU(Layer):
    ...
    def backward(self, dout):
        ...
        # ====== YOUR CODE: ======
        local=torch.ones_like(x)
        local[x<0]=self.alpha
        dx=dout*local
        # ========================
...
class TanH(Layer):
    ...
    def forward(self, x, **kw):
        ...
        # ====== YOUR CODE: ======
        e_x = torch.exp(x)
        e_minus_x = torch.exp(-x)
        out = (e_x-e_minus_x)/(e_x+e_minus_x)
        self.grad_cache["out"] = out
...
class CrossEntropyLoss(Layer):
    ...
    def forward(self, x, y):
        ...
        log_sum = torch.log(sum_exp_x)
        x_y = x[torch.arange(N),y]
        loss_vector = -x_y + log_sum
...
class MLP(Layer):
    ...
    def __init__(
        ...
        **kw,
        ...
        for hid in hidden_features:
            layers.append(Linear(in_features=in_features,out_features=hid))
            if activation == "relu":
                layers.append(ReLU())
            else:
        ...
        layers.append(Linear(in_features=in_features,out_features=num_classes))
